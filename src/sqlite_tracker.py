...
class DatabaseManager:
            ...
            with self.transaction():
                ...
                if 'hash_info' not in columns:
                    ...
                    self.cursor.execute("""
                        ...
                        SELECT image_name, phash FROM image_hashes
                    """)
    ...
    def add_image_hash(self, image_name, phash):
        with self.transaction():
            self.cursor.execute(
                "INSERT OR REPLACE INTO image_hashes (image_name, phash) VALUES (?, ?)",
                (image_name, phash)
            )
    ...
    def add_image_hash_with_info(self, image_name, phash, hash_info):
        with self.transaction():
            self.cursor.execute(
                ...
                (image_name, phash, json.dumps(hash_info))
            )
    ...
    def is_image_processed(self, image_name):
        try:
            self.cursor.execute(
                "SELECT 1 FROM processed_images WHERE image_name = ? LIMIT 1",
                (image_name,)
            )
            return self.cursor.fetchone() is not None
        except sqlite3.Error as e:
    ...
    def is_image_downloaded(self, image_id):
        try:
            self.cursor.execute(
                "SELECT 1 FROM downloaded_images WHERE image_id = ? LIMIT 1",
                (image_id,)
            )
            return self.cursor.fetchone() is not None
        except sqlite3.Error as e:
    ...
    def add_event_title(self, title):
        with self.transaction():
            self.cursor.execute(
                "INSERT OR IGNORE INTO event_titles (title) VALUES (?)",
                (title,)
            )
        ...
        try:
            ...
            with self.transaction():
                self.cursor.execute(
                    ...
                    (id, summary, dtstart, location) VALUES (?, ?, ?, ?)""",
                    (event_id, event_data['SUMMARY'], 
                     str(event_data['DTSTART']), event_data['LOCATION'])
                )
    ...
    def is_event_sent(self, event_id):
        try:
            self.cursor.execute(
                "SELECT 1 FROM sent_events WHERE event_id = ? LIMIT 1",
                (event_id,)
            )
            return self.cursor.fetchone() is not None
        except sqlite3.Error as e:
