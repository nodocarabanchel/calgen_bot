            ...
            for component in gcal.walk():
                if component.name == "VEVENT":
                    try:
                        ...
                        end = component.get('DTEND').dt if component.get(
                            'DTEND') else None
                        ...
                        if recurrence:
                            ...
                            adjusted_start = get_next_valid_date(
                                start, rrule_string)
                        ...
                        logger.info(
                            f"Event details extracted: {event_details}")
...
def send_event(config, event_details, base_filename, image_path=None, max_retries=5):
    ...
    if 'end_datetime' in event_details:
        ...
        data['multidate'] = 'true' if (
            end_datetime.date() - start_datetime.date()).days > 0 else 'false'
    else:
        logger.info(
    ...
    if image_path and Path(image_path).exists():
        ...
        if img_type in ['jpeg', 'png', 'gif']:
            ...
            files['image'] = (f'image.{img_type}',
                              compressed_image, f'image/{img_type}')
    ...
    while retries < max_retries and total_wait_time < max_wait_time:
        try:
            ...
            if response.status_code == 200:
                logger.info(
                    f"Successfully sent event: {event_details['title']}")
                return response
            elif response.status_code == 404:
                logger.error(
                    f"404 Not Found: The endpoint {api_url} does not exist.")
                break
